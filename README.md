...
> 전체 화면과 사용자 흐름 UI는 구현이 완료된 상태이며, 현재는 도메인별로 실제 API 연동을 진행하며 기능을 구체화하고 있습니다. Dashboard(주간)와 일지 모아보기는 이미 실제 API로 전환되었고, 나머지 도메인도 순차적으로 연동할 예정입니다. (자세한 진행 상황은 [API 연동 현황](#api-연동-현황) 참고)
